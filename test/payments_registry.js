...
contract('MockPaymentsRegistry', function(accounts) {
    ...
    describe("when processing a payment", () => {
        ...
        let subscriptionHash = await newSubscription(subscriptionContract, tokenContract.address, accounts[0], "process.valid");
        let result = await paymentsRegistry.createNewPayment(subscriptionHash, subscriptionContract.address, oneMonthLater, 400, {from: accounts[0]});
        ...
        it("should not be able to set the duedate in the past", async function() {
            ...
            await assertRevert(paymentsRegistry.processPayment())
    ...
    describe("when removing a claimant", () => {
        ...
        let subscriptionHash = await newSubscription(subscriptionContract, tokenContract.address, accounts[0], "remove");
        let result = await paymentsRegistry.createNewPayment(subscriptionHash, subscriptionContract.address, oneMonthLater, 400, {from: accounts[0]});
