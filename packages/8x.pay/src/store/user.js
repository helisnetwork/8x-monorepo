...
export default class UserStore {
  ...
  listenExchangeRate() {
    bus.on('request:exchange:rate', () => {
      ...
      kyberNetworkProxy.getExpectedRate(src, token, srcQty, function(err, result) {
        let exchangeRate = new BigNumber(result[0]);
        // let slippageRate = new BigNumber(result[1]);

        if(!err) {
          bus.trigger('exchange:rate:sent', exchangeRate/new BigNumber(10).pow(18));
          console.log(exchangeRate/new BigNumber(10).pow(18));
          // console.log(slippageRate/new BigNumber(10).pow(18));
  ...
  listenUserConversion() {
    bus.on('conversion:requested', async (selectedPeriodInMonths) => {
      ...
      bus.on('subscription:plan:sent', (planData) => {
        ...
        const priceInDAI = ((selectedPeriodInMonths * 30 * 24 * 60 * 60)/(planData.subscriptionPeriod))*(planData.subscriptionAmount);

        bus.on('exchange:rate:sent', (rate) => {
          const DAItoETH = (priceInDAI/rate);
          const kyberNetworkProxyInterface = '0x7e6b8b9510D71BF8EF0f893902EbB9C865eEF4Df';
          const token = '0xB2f3dD487708ca7794f633D9Df57Fdb9347a7afF'; //KNC KOVAN COIN
          ...
          kyberNetworkProxy.swapEtherToToken(token, minConversionRate, { value: srcAmount }, function (err, results) {
            if(!err) {
