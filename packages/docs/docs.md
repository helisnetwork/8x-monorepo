# Plans
 ...
 The plans API primarily provides functionality for a business to create, get and cancel subscription plans.
...
The subscriptions api provides all the functionality for consumers to subscribe to 8x. A user must first subscribe to a subscription,
...
```typescriptimport eightEx from 'eightEx';const eightEx = new EightEx(web3, addressBook);eightEx.subscriptions.hasGivenAuthorisation(   owner: Address): Promise<boolean> ```
...
```typescriptimport eightEx from 'eightEx';const eightEx = new EightEx(web3, addressBook);eightEx.subscriptions.giveAuthorisation(   txData: TxData): Promise<TxHash> ```
...
```typescriptimport eightEx from 'eightEx';const eightEx = new EightEx(web3, addressBook);eightEx.subscriptions.subscribe(   planHash: Bytes32,   metaData: (JSON | null),   txData: TxData): Promise<Bytes32> ```
...
### Request ParametersName | Type | Comment ---- | ---- | ------- planHash | Bytes32 | Unique identifying hash of the plan. metaData | (JSON &#x7c; null) | Any extra JSON data you'd like to pass/store on-chain (optional). txData | TxData | Provide signer, gas and gasPrice information (optional). ## Activate
`activate`
```typescriptimport eightEx from 'eightEx';const eightEx = new EightEx(web3, addressBook);eightEx.subscriptions.activate(   subscriptionHash: Bytes32,   txData: TxData): Promise<TxHash> ```
